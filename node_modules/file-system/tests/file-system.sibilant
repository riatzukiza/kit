  ...
  (let ((file-system ((create File-system) ".")))
    ...
    (pipe (.resolve Promise)
          ...
          (p-test "find always returns the same node for the same file"
                  (.all Promise
                        [
                        (.find file-system "./sibilant/index.sibilant")
                        (.find file-system "./sibilant/index.sibilant")
                        ])
          ...
          (p-test (.watch file-system "./hello.txt") "watch events for the same file, consistantly return the same node"
                  (then node [(once-then "add" node) node])
