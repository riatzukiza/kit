...
(def run-in-sandbox (js input-path)
     ...
     (set require.main
          ...
          'paths (pipe input-path fs.realpath-sync path.dirname mod._node-module-paths))
...
(when (and (or (undefined? cli-options.file) (empty? cli-options.file)) cli-options.docs)
      ...
      (pipe sibilant.docs
          (get (pipe cli-options.docs first sibilant (get 'js)))
          call
          console.log))
...
(each input-file (or cli-options.file [])
      (var input-path (if (match-regex? input-file "^\\/")
           ...
           map? (pipe cli-options.sourcemap not not)
